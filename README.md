# Hi there 👋

I'm a .NET developer with a love for coffee, programming, and biotechnology.

## Most recent project (Work-in-progress)
- Distributed Compilation (but it makes sense): [HPCC](https://github.com/aarani/hpcc)
- On-demand minecraft server hosting (using Firecracker with eBPF based networking): [Craftling](https://github.com/GoCraftling/Craftling)

## Some of my work:
- Managed implementation of TOR protocol [NOnion](https://github.com/aarani/NOnion/commits?author=aarani)
- [NBlockchain\geewallet (Lightning)](https://github.com/nblockchain/geewallet/commits/rc/LN-m13?author=aarani)
- [NBlockchain\DNL.Kiss](https://github.com/nblockchain/DotNetLightning.Kiss/commits?author=aarani)

## Some of my fun side-projects
- Function as a service (POC): [MangoFaaS](https://github.com/aarani/MangoFaaS)
